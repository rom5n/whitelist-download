...
- **Скрытие конфигураций в отдельную папку**
...
#### 1. Установка (Windows)
1. Скачайте и запустите .exe файл. Программа сама создаст нужные файлы и запустит локальный HTTP-сервер.

#### 1. Установка (Linux)
1. В терминале рядом с файлом выполните:
```bash
...
```Bash
chmod +x wl-download-mac-arm64 # Замените arm на amd, если скачали amd версию
```
7. Запустите файл командой `./wl-download-mac-arm64` (Также замените arm на amd, если скачали amd версию).
...
yarn build
cd ..\backend
go build -tags with_utls -ldflags="-s -w -H=windowsgui" -o "wl-download.exe" .
```
...
- 📝 `app.log` — Логи программы.
- 🛠️ `config.json` — Настройки проекта.
- ⚙️ `wl-download-windows.exe` — Основной бинарный файл. **Не перемещайте его после запуска!**. Если все же нужно переместить, то найдите в диспетчере задач процесс 'wl-download.exe' и завершите его, после переместите файл в новое место и запустите его.
